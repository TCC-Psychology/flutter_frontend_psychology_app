...
class _PsychologistEditProfileScreen
  ...
  final UserProfileService userService = UserProfileService();
  ...
  fetchUser() async {
    ...
    var fetchedUser = await userService.fetchUserByUserId(idUserLogged);
    if (fetchedUser != null) {
      user = fetchedUser;
      setState(() {
        name.text = user?.name ?? "";
  ...
  updateUser() async {
    ...
    UserProfile userUptaded = UserProfile(
      name: name.text,
      city: city.text,
    ...
    await userService
        .updateUserById(idUserLogged, userUptaded)
